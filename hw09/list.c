...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    if (((new_user = (struct user *) malloc(sizeof(struct user))) == NULL)) {
        free(new_user);
        return NULL;
    }
    if (type == INSTRUCTOR) {
        struct instructor *new_instructor = (struct instructor *) malloc(sizeof(struct instructor));
        if (new_instructor == NULL) {
            free(new_instructor);
            free(new_user);
            return NULL;
        }
        if (create_instructor(data.instructor.salary, new_instructor) == FAILURE) {
            free(new_instructor);
            free(new_user);
            return NULL;
        }
        if (name == NULL) {
            ...
            if (namechar == NULL) {
                ...
                new_user->name = namechar;
                strncpy(new_user->name, name, sizeof(char) * (strlen(name) + 1));
            }
        }
        new_user->data.instructor = *new_instructor;
        new_user->type = INSTRUCTOR;
        free(new_instructor);
        return new_user;
    }
    if (type == STUDENT) {
        ...
        if (create_student(data.student.num_classes, data.student.grades, new_student) == FAILURE) {
            free(new_student);
            free(new_user);
            return NULL;
        }
        ...
        new_user->data.student = *new_student;
...
struct user_list *create_list(void)
{
    ...
    if ((u_list = malloc(sizeof(struct user_list)))) {
        u_list->head = NULL;
        u_list->size = 0;
        return u_list;
    }
...
int push_back(struct user_list *list, char *name, enum user_type type, union user_data data)
{
    ...
    struct user *new_user;
    new_user = create_user(name, type, data);
    if (new_user == NULL) {
        return FAILURE;
    }
    if (list->head != NULL) {
...
int add_at_index(struct user_list *list, int index, char *name, enum user_type type, union user_data data)
{
    ...
    struct user *dummy1 = list->head;
    struct user *dummy2 = NULL;
    while (index > 0) {
        dummy2 = dummy1;
        dummy1 = dummy1->next;
        index--;
    ...
    if (dummy2) {
        dummy2->next = new_user;
        new_user->next = dummy1;
    } else {
        new_user->next = list->head;
        list->head = new_user;
    }
...
int contains(struct user_list *list, struct user *data, struct user **dataOut)
{
    ...
    struct user *dummy = list->head;
    while(dummy != NULL && !user_equal(data, dummy)) {
        dummy = dummy->next;
    }
    if(dummy == NULL) {
        *dataOut = NULL;
...
int pop_front(struct user_list *list, struct user **dataOut)
{
    ...
    *dataOut = list->head;
    struct user *next_node = list->head->next;
    if(!next_node) {
        list->head = NULL;
    } else {
        list->head = next_node;
...
int remove_at_index(struct user_list *list, struct user **dataOut, int index)
{
    ...
    *dataOut = dummy->next;
    struct user *temp_user = dummy->next->next;
    dummy->next = temp_user;
    list->size--;
    return SUCCESS;
}
...
void empty_list(struct user_list *list)
{
    ...
    struct user *dummy = list->head;
    while(dummy != NULL) {
        struct user *dummy_next = dummy->next;
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
    ...
    *dataOut = NULL;
    double maxSalary = 0;
    while(dummy) {
        if(dummy->type == INSTRUCTOR) {
            if(dummy->data.instructor.salary > maxSalary) {
                maxSalary = dummy->data.instructor.salary;
                *dataOut = dummy;
            }
        }
...
int is_passing_all_classes(struct user_list *list, char *name, int *dataOut)
{
    ...
    struct user *dummy = list->head;
    while(dummy) {
        if(dummy->type == STUDENT && (strcmp(dummy->name, name) == 0)) {
            if(dummy->data.student.num_classes == 0) {
                *dataOut = 0;
                return SUCCESS;
            }
            for(int i = 0; i < dummy->data.student.num_classes; i++) {
                if(dummy->data.student.grades[i] < 60) {
                    *dataOut = 0;
                    return SUCCESS;
                }
            }
...
int end_semester(struct user_list *list)
{
    ...
    struct user *dummy = list->head;
    while(dummy) {
        struct user *temp = dummy->next;
        if (dummy->type == INSTRUCTOR) {
            dummy->data.instructor.salary += 10000;
        }
        if(dummy->type == STUDENT) {
            dummy->data.student.num_classes = 0;
            free(dummy->data.student.grades);
            dummy->data.student.grades = NULL;
        }
        dummy = temp;
    }
    return SUCCESS;  
