...
LDR R0, R5, 4           ;R0 = address of head
ST R0, FIRSTNODE
LDR R1, R5, 5           ;R1 = exp
LDR R2, R5, 6           ;R2 = func

WHILE   ADD R0, R0, 0   ;Check if R0 == 0
        BRz ENDWHILE
        LDR R4, R0, 1   ;Get data from head
        ADD R6, R6, -2  ;Clear 2 slots for 
        STR R4, R6, 0   ;Store head.data to R4
        STR R1, R6, 1   ;Store exp to R1
        JSR pow         ;Jump to Subroutine Powers
        LDR R4, R6, 0   ;Load head.data to R4
        LD R3, FINISHEDSUM
        ADD R4, R3, R4
        ST R4, FINISHEDSUM
        LDR R0, R0, 0
